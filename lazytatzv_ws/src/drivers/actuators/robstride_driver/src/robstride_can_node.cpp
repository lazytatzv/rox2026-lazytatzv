...
RobstrideCanNode::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  publisher_can_tx_ =
    this->create_publisher<seeed_usb_can_analyzer_driver::msg::CanFrame>("/communication/tx_queue",
      10);
...
void RobstrideCanNode::can_rx_callback(
  const seeed_usb_can_analyzer_driver::msg::CanFrame::SharedPtr message)
{
  if (this->get_current_state().id() != lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {return;}
